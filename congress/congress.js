...
const missedVotesRep = getSimplifiedCongress(representatives).filter((rep) => rep.title === 
"Representative").reduce((acc, rep) => acc.missed_votes_pct > rep.missed_votes_pct ? acc : rep)

const partyHack = getSimplifiedCongress(representatives).filter((rep) => rep.title === 
'Representative').reduce((acc, rep) => acc.votes_with_party_pct > 
rep.votes_with_party_pct ? acc : rep)
